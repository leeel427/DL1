'''
    ...
    4. 퍼셉트론(perceptron) 등장
        1) 생물학적 뉴런을 모방하여 만든 인공신경망의 기본 단위
            - 이전 perceptron 과 연결 ==> inputs - weights - input_function - activation_function - output
                         ==> 다음 perceptron 으로 전달
            - 입력 ----> 모두 더함 ---> 함수 ----> 출력
                    w
            - 딥러닝은 가중치(Wn)를 수정하는 행위
            - 뇌세포는 항상 일정한 크기의 값을 출력함 (0 or 1)
            - 뇌세포의 출력물은 디지털 신호로 간주할수 있음
        ...
        2) activation function (활성화 함수)
            ...
            - 하이퍼볼릭 탄젠트 함수 : (음수값 존재) 최근 음수값을 사용하지 않으려는 추세로 선호도 낮아짐
            - 렐루 함수 : 딥러닝 성능을 크게(꾸준히) 향상시키는 경향이 있어 널리 활용됨
            - 리키 렐루 함수 : 음수값이 정보가 일부 보정되어, 렐루보다 성능이 뛰어남
        ...
        4) AND / OR 연산
            - 초기 퍼셉트론을 이용한 문제해결은 AND 와 OR 같은 간단한 연산이었음
                - 기계가 AND와 OR 연산을 스스로 풀 수 있으면 이를 조합해 어떤 문제든 풀어낼수 있다고 생각했음.
        ...
        5) XOR 연산
            ...
            - 각각의 가중치(Weight)와 편향(절편,Bias)을 학습시킬 방법의 한계 언급 => 침체기를 맞이함
